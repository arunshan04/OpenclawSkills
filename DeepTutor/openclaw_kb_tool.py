...
def _safe_json_dumps(data: Any) -> str:
    """Serialize logging payloads without truncating content."""
    try:
...
def _http_post_json(url: str, payload: Dict[str, Any], timeout: int = 300) -> Dict[str, Any]:
    ...
    logger.info("HTTP_REQUEST %s", _safe_json_dumps({
        "method": "POST",
        "url": url,
        "payload": payload,
        "timeout": timeout,
    ...
    with urllib.request.urlopen(request, timeout=timeout) as response:
        ...
        logger.info("HTTP_RESPONSE %s", _safe_json_dumps({
            "method": "POST",
            "url": url,
            "response": result,
...
def query_knowledge_base(
    ...
    mode: str = "hybrid",
    base_url: str = "http://192.168.10.101:8090",
    ...
    Args:
        ...
        base_url: Base URL of the running Knowledge Base API server (default: http://192.168.10.101:8090)
    ...
    try:
        ...
        logger.info("QUERY_REQUEST %s", _safe_json_dumps(payload))
        ...
        if result.get("success", False):
            ...
            logger.info("QUERY_RESPONSE %s", _safe_json_dumps({
                "query": query,
                "kb_name": kb_name,
                "mode": mode,
                "session_id": sid,
            ...
            logger.error("QUERY_RESPONSE %s", _safe_json_dumps({
                "query": query,
                "kb_name": kb_name,
                "mode": mode,
                "session_id": session_id,
...
def list_knowledge_bases(base_url: str = "http://192.168.10.101:8090") -> str:
    """
...
def query_kb_chunks(
    ...
    try:
        ...
        logger.info("CHUNKS_REQUEST %s", _safe_json_dumps(payload))
        result = _http_post_json(f"{base_url}/query-chunks", payload, timeout=600)
        logger.info("CHUNKS_RESPONSE %s", _safe_json_dumps({
            "query": query,
            "kb_name": kb_name,
            "mode": mode,
            "top_k": top_k,
...
def get_kb_info(kb_name: str, base_url: str = "http://192.168.10.101:8090") -> str:
    """
...
def get_kb_status(base_url: str = "http://192.168.10.101:8090") -> str:
    """
...
def list_chat_sessions(base_url: str = "http://192.168.10.101:8090", limit: int = 20) -> str:
    """
...
def get_chat_session(session_id: str, base_url: str = "http://192.168.10.101:8090") -> str:
    """
...
def create_openclaw_tool(base_url: str = "http://192.168.10.101:8090", default_kb: Optional[str] = None):
    """
...
Examples:
  ...
  python openclaw_kb_tool.py info "NVIDIA"
    ...
    parser.add_argument(
        "--url",
        default="http://192.168.10.101:8090",
        help="API server URL (default: http://192.168.10.101:8090)"
    )
